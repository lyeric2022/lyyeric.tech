...
export const content = (
  <>
    ...
    <p>
      Sometimes I think I'm quite clever / good at using LLMs to solve problems. But it turns out everyone, everywhere, is just vibe-coding too. Here are some of my learnings though :D
    </p>
    <br />

    <h2>Judgment and velocity</h2>

    <p>
      In practice, by chaining web search + vector/fs search + commit checks + logs + skills + general intuition, I'm able to effectively build anything. Maybe a year ago, models weren't reliable enough, but today, they honestly kinda are. It kinda sucks in some sense that I would've been much better syntactically and detail-wise without LLMs, but the speed at which I can build and fix problems is godlike compared to any other timeline.
    </p>
    <br />

    <p>
      On speed, it's quite freaky to see agents refactor tens of thousands of lines of code within a few hours. The ability of Composer 2 to build so quickly is astonishing. The problems that I have now are more about: 1) what to build—does this actually improve the UX, what are the costs, latency, and alternatives—and 2) what tools to use. I default to TypeScript, Next.js, Vercel, Node, Prisma, and I deploy to Render. If I'm using Python, then I'd go with SQLAlchemy + Alembic.
    </p>
    <br />

    <h2>Stack and vendors</h2>

    <p>
      Elsewhere in the default stack, TanStack, Infisical, Redis, and PostHog are very useful, and for DB, I've just been defaulting to Supabase, although Neon & Convex seem to be gaining a lot of popularity. For agent frameworks, Letta's quite awesome, although AI SDK is now the default choice for TypeScript. Tailwind is way too standard now, and I don't really think about much else for UI. Devin's code review is quite nice. CI for DB migration builds is mandatory, and Linear is the greatest for tickets. E2E pipeline testing is ideal, and so are the builds for FE/BE.
    </p>
    <br />

    <p>
      I should also give more credit to PostHog. Sentry is quite the default for many orgs, but I just love how I can do everything on PostHog: analytics, feature flags, request logs, session replays.
    </p>
    <br />

    <p>
      More broadly, my philosophy is minimizing the number of vendors, so as to keep all projects as maintainable as possible. Supabase supports so much: general Postgres, pgvector, vaults, auth. That really sucked when connection limits were maxed and auth failed alongside it.
    </p>
    <br />

    <h2>Hygiene through production</h2>
    ...
    <p>
      For release, unit tests are mandatory for ensuring proper validation behavior. Deployments really depend on the number of customers & needs, and I generally dislike over-engineering. For most stuff though, I like to have separate branches for production, demo, staging, development, and localhost.
    </p>
    <br />

    <p>
      On infra and packaging, I've recently learned about the need for worker services to be unbundled from the main server, how easy it is to upload packages for npm/PyPI, and that Docker instances should have different tags when trying to run concurrent builds, as well as issues related to race conditions, queue pollution, and process resumability.
    </p>
    <br />
    ...
    <p>
      On MCP specifically, the picture may look very different in 2 years. Letta believes that shell-based actions will triumph over browser-based / MCP-based actions, and Composio agrees too, as they're now also building in the sandbox space. MCP tools fall somewhere between being too context-pollutive and not dynamic enough, and Pointer's browser tools had recurring latency + inaccuracy.
    </p>
    <br />
    ...
    <p>
      In that same vein, software engineers are increasingly becoming just context engineers, and spec-driven development is quite popular at large orgs. I'm sure I've used this philosophy in some form, but perhaps agents maintaining a living record of past learnings and preferences may go further.
    </p>
    <br />
    ...
    <p>
      On evals, using an LLM as a judge is decent and cheap. Solve real problems & research novelty. Something interesting comes out literally every week now.
    </p>
  </>
);
